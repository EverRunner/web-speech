...
export default function HoverPlayer() {
  ...
  const handlePlayText = () => {
    if (hoveredParagraph?.element) {
      speechify(hoveredParagraph.element);
    }
  };

  if (!hoveredParagraph) return null;

  const { top, left } = hoveredParagraph;
  // TODO: (YildirimE) heightOfFirstLine would be used to transform PlayButton after correct it
